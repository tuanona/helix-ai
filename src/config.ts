...
const { values } = parseArgs({
  ...
  options: {
    ...
    geminiKey: {
      type: "string",
      default: Bun.env.GEMINI_API_KEY,
    },
    geminiModel: {
      type: "string",
      default: Bun.env.GEMINI_MODEL ?? "gemini-1.5-flash",
    },
    geminiContext: {
      type: "string",
      default: Bun.env.GEMINI_CONTEXT?.length
        ? Bun.env.GEMINI_CONTEXT
        : context.gemini,
    },
    geminiEndpoint: {
      type: "string",
      default:
        Bun.env.GEMINI_ENDPOINT ?? "https://generativelanguage.googleapis.com",
    },
    geminiMaxTokens: {
      type: "string",
      default: Bun.env.GEMINI_MAX_TOKENS ?? "8192",
    },
  },
  strict: true,
...
if (
  ...
  !values.geminiKey?.length &&
  !values.authCopilot &&
  !values.authCodeium &&
  values.handler !== "codeium" &&
  values.handler !== "ollama" &&
  values.handler !== "gemini"
) {
